...
wathNodeHost(){
  local text ret=0
  while true; do
    text="$(kubectl get pod "${NAME}-leader-0" -n "${NAME_SPACE}" -o jsonpath={".status.podIP"})"
    if [[  -n "$text" ]]; then
      echo "$text"
      return
    fi
...
wathCreateCluster() {
  while [[ "$(docker exec -t redis-benchmark redis-cli -h "$1" -a "$PASSWD" --no-auth-warning cluster nodes | wc -l)" != "$REPLICAS_COUNT" ]] ;do
    sleep 5
  done
}
...
for yaml in ${SCRIPT_PATH}/redis-cluster.yaml.d/*.yaml; do
  # 部署新集群
  basenameYaml=$(basename "${yaml%.yaml}")
  echo "部署: ${yaml}"
  kubectl apply -f "$yaml"
  node=$(wathNodeHost)
  wathCreateCluster "${node}"
